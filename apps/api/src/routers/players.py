...
def _run_report_generation(
    ...
    try:
        ...
        result = db.execute(select(PlayerGame).where(PlayerGame.id.in_(game_ids)))
        games = list(result.scalars().all())

        result = db.execute(select(PlayerReport).where(PlayerReport.id == report_id))
        report = result.scalar_one_or_none()

        if not player or not report:
            logger.error(
                "Background report generation: player or report not found",
...
@router.delete(
    "/{player_id}/reports/{report_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
async def seed_demo_players(
    ...
    demo_players = [
        ...
        # PLAYER 2: Playmaking PG — triple-double, assists leader, working on TOV
        {
            ...
            "goals": [
                "Reduce turnovers to under 3 per game",
                "Make varsity",
                "Improve leadership",
            ],
        ...
        # PLAYER 4: Dominant C — double-doubles every game, rim protector
        {
            ...
            "goals": [
                ...
                "Improve free throws to 70%+",
                "Get stronger in the post",
            ],
            "competition_level": "Varsity",
        ...
        # PLAYER 5: Developing freshman — improvement arc, early milestones
        {
            ...
            "goals": [
                "Make JV team",
                "Improve shooting to 40%+ FG",
                "Get more confident on the floor",
            ],
